...
class Odom2PoseNode:
    ...
    def callback_enco(self, sensor_state):
        ...
        right_speed = 2*np.pi*(d_right_encoder/self.ENCODER_RESOLUTION)
        left_speed = 2*np.pi* (d_left_encoder/self.ENCODER_RESOLUTION)

        self.v = self.WHEEL_RADIUS*(right_speed+left_speed)/2
        self.w = self.WHEEL_RADIUS*(right_speed-left_speed)/self.ENTRE_AXE

        # TODO: Update x_odom, y_odom and O_odom accordingly

        self.x_odom += np.cos(self.O_odom)*self.v
        self.y_odom += np.sin(self.O_odom)*self.v
        self.O_odom += self.w
